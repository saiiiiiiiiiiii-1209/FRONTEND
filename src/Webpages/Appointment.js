...
//             <tr>
//               <th>Sr. No.</th>
//               <th>Appointment ID</th>  {/* ✅ New column added */}
//               <th>Patient</th>
//               <th>Age/Gender</th>
//               <th>Phone</th>
//               <th>Email</th>
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        const processedAppointments = (data.appointments || []).map(apt => ({
          ...
          id: apt._id,
          _id: apt._id
          // appointmentId is already there from backend, but we'll keep it for internal use
        }));
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        const newAppointment = {
          _id: data.appointment._id,
          id: data.appointment._id,
          appointmentId: data.appointment.appointmentId, // Keep for internal use only
          patientName: data.appointment.patientName,
          age: data.appointment.age,
          gender: data.appointment.gender,
          phone: data.appointment.phone,
    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter((apt) => {
        return (
          apt.patientName?.toLowerCase().includes(searchLower) ||
          apt.phone?.includes(searchTerm) ||
          // apt.appointmentId?.toLowerCase().includes(searchLower) || // Commented out - search by ID internally but don't display
          apt.email?.toLowerCase().includes(searchLower)
        );
      });
    }
      ...
      <div className="table-container">
        <table className="data-table">
          <thead>
            <tr>
              <th>Sr. No.</th>
              {/* <th>Appointment ID</th> */}  {/* ❌ REMOVED - Hidden from UI */}
              <th>Patient</th>
              <th>Age/Gender</th>
              <th>Phone</th>
              <th>Email</th>
          ...
          >
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
              <div style={{ gridColumn: "span 2", background: "#f8f9fa", padding: "15px", borderRadius: "8px" }}>
                <h3 style={{ margin: "0 0 15px 0", color: "#0d6efd", fontSize: "16px" }}>Appointment Information</h3>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "15px" }}>
                  {/* <div><strong>Appointment ID:</strong> {selectedAppointment.appointmentId || 'N/A'}</div>  ❌ REMOVED - Hidden from UI */}
                  <div><strong>MongoDB ID:</strong> <small>{selectedAppointment._id}</small></div>
                  <div><strong>Status:</strong> 
                    <span style={{
                      marginLeft: "8px",
